...
const Login = () => {
    ...
    let errorMessage = ''
    if (error) {
        const userErr = error?.message.includes('user-not-found')
        const passwordErr = error?.message.includes('wrong-password')
        if (userErr) {
            errorMessage = 'Your email not registered. Please Registered first..'
        }
        if (passwordErr) {
    ...
    return (
        <div className='py-20 bg-slate-100 px-5'>
            ...
            <div className='md:w-1/3 mx-auto'>
                ...
                {errorMessage}
                <p className='text-xl mt-10 mb-3'>Don’t have an account? <Link className='font-bold' to='/register'> Sign up now </Link> </p>
